...
final class DTLN2Processor: AudioProcessor {
    ...
    private func configureSessions() throws {
        ...
        // The reference DTLN ONNX models use external LSTM state with the
        // fixed shape [1, 2, 128, 2]. The reference implementation allocates
        // this from the model input shape; keeping it explicit here avoids
        // depending on package-version-specific metadata APIs.
        state1Shape = [1, 2, 128, 2]
        state2Shape = [1, 2, 128, 2]
        state1 = [Float](repeating: 0, count: 1 * 2 * 128 * 2)
        state2 = [Float](repeating: 0, count: 1 * 2 * 128 * 2)
    }
